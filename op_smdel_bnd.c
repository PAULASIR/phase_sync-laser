...
FILE *fp1,*fp2,*fp3;
fp1=fopen("par_tau_bnd.dat","w");

 
       for( tau_phi =-0.4; tau_phi >= -0.415; tau_phi-= 0.0005){ 
            for(eps =0.1; eps <= 0.2; eps+= 0.0005){
...
sm =0.0;  sm1 = 0.0;  syn_error[i]=0.0;
for(k=1;k<=20000;k++)
	{               
        t=h*(double)(k);
    	RK4(nn,n,h,t,y,DGL);   
...
/**************************************************/    
   ...
   if( sn_sum(syn_error,nn) >= 0.05  && sn_sum(syn_error,nn) <=0.1 )
   {fprintf(fp1,"%20.12lf   %f   %20.12lf \n", tau_phi, eps, sn_sum(syn_error,nn));
   break;}
  }
}
