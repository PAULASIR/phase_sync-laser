...
FILE *fp1,*fp2,*fp3;
fp1=fopen("par_psd1.dat","w");

 
for( omega =1.0; omega <= 12.0; omega+= 0.5){
    for(eps =0.0; eps <= 0.5; eps+= 0.025){
...
/*----------------max & min of the series--------
   ...
   double psd = (1.0/(float) 2.0*M_PI)* creal(fabs(z[i]))* cexp(-I*2.0*M_PI*omega);
