...
FILE *fp1,*fp2,*fp3;
fp1=fopen("par_tau_corr2.dat","a");


for(delta = 0.0;  delta<= 0.5; delta+=0.1){ 
for( tau_phi =-0.0; tau_phi >= -1.5; tau_phi-= 0.05){
    for(eps =0.0; eps <= 0.3; eps+= 0.05){
...
/**************************************************/    
   ...
   //  getsum1(zz,nn);
   fprintf(fp1,"%20.12lf   %20.12lf   %20.12lf  %20.12lf \n", delta, tau_phi, eps, avg_syn);
  }
}
}
  printf("process over!!\n");
	   ...
	   void (*DGL)(double,double[101][501],double[101][501]))
{
	   ...
	   {
             for(i=1;i<=n;i++)
	      y[j][i]=y[j][i]+h*((k1[j][i]+2.0*k2[j][i]+2.0*k3[j][i]+k4[j][i])/6.0);
	   }
}
//*********************FUNCTION SUBROUTINE********************************//
