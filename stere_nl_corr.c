...
double x_max,x_min;
int l;
double sm_theta, sr_angle, sm_sra;
double theta[30000];
srand(time(0));
...
//fp3=fopen("ldl_cc.dat","w");
 fp4 = fopen("area_delta.dat","w");
 
 
for(eps = 0.0; eps <= 1.0; eps += 0.05){
 for(delta = 0.0; delta <=1.0; delta+=0.1){
 printf("%f   %f\n", eps,delta);
 
sm_theta = 0.0;   sm_sra = 0.0;
...
/*----------------max & min of the series--------
   ...
   if(fabs(z1[k] - z1[k-1]) <= 1e-3)
       t1 = t;
       else if(fabs(z1[k] - z1[k-1]) >= 1e-3)
       t2 = t;
      ...
      // printf("%f  %f   %f    %f\n", theta[k], theta[k-1], z1[k], z1[k-1]);
        sm_theta += fabs(theta[k]- theta[k-1]);
       
     //  sr_angle = 2.0*M_PI*1.0-cos(sm_theta);
