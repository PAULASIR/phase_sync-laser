/* Lang-Kobayashi equations delayed autocorrelatively coupled laser modes */
/* Author: Paul Asir */
#include<stdio.h>
#include<math.h>
#include<stdlib.h>
...
int i,j,k,nn,n=5;
double y[MX_NX][MX_NY], x_tau, r_tau, J_0,g , R_sp, s, G, alpha;
...
void RK4(int,int,double,double,double[MX_NX][MX_NY],
                   void (*DGL)(double,double[MX_NX][MX_NY],double[MX_NX][MX_NY]));
void DGL(double, double[MX_NX][MX_NY],double[MX_NX][MX_NY]);
...
/**************************************************/    
   ...
   printf("%20.20lf \n",  sn_sum(syn_error,nn));
    ...
    // getsum(z,nn);
     ...
     fprintf(fp2,"%d  %20.20lf\n", nn, fabs(sn_sum(syn_error,nn)));
   //  getsum1(zz,nn);
...
//************************RK4 SUBROUTINE*********************************//
void RK4(int nn,int n,double h,double t,double y[MX_NX][MX_NY],
	   void (*DGL)(double,double[MX_NX][MX_NY],double[MX_NX][MX_NY]))
{
	   
	   double k1[MX_NX][MX_NY],k2[MX_NX][MX_NY],k3[MX_NX][MX_NY],k4[MX_NX][MX_NY];
	   double yaux[MX_NX][MX_NY];
...
//*********************FUNCTION SUBROUTINE********************************//
void DGL(double t,double y[MX_NX][MX_NY],double F[MX_NX][MX_NY])
{
   ...
   for(j=1;j<=nn;j++)
     ...
     for(i=1;i<=nn;i++)
      {
         if(i != j && abs(i-j)%(nn-(int)(p*nn))<=(int)(p*nn)) 
           aa[j][i]=1;
          else
            aa[j][i]=0; 
